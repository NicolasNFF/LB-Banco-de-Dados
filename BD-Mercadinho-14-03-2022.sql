...
(@login varchar(50), @senha varchar(100)) AS
DECLARE @RETORNO INT
SELECT @RETORNO = COUNT(USU_IN_CODIGO) FROM MER_USUARIO		
				  WHERE USU_ST_LOGIN = @login AND
				        USU_BI_SENHA =
	   HASHBYTES('SHA2_512',CONVERT(VARBINARY,@senha))
IF @RETORNO < 1
BEGIN
	RAISERROR('O usuário ou a senha informados são  inválidos!',15,1)
    RETURN
END
PRINT @RETORNO
RETURN

SP_S_MER_USUARIO_LOGIN 'mlima','secreta123' 
...
IF LEN(@codigo) < 1
BEGIN
	RAISERROR('É obrigatorio informar o código do usuario!',15,1)
	RETURN
END
DELETE FROM MER_USUARIO WHERE USU_IN_CODIGO = @codigo;
IF @@ROWCOUNT = 0 --retorna o nr de linhas 'afetadas'
BEGIN
	RAISERROR('Não havia nenhum usuário para excluir com código informado',15,1)
END
RETURN
GO
SP_D_MER_USUARIO 152; --VAI DAR ERRO. NÃO EXISTE 152
SP_D_MER_USUARIO 1; --É PRA EXCLUIR COM SUCESSO
